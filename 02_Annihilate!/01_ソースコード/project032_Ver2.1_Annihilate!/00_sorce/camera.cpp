...
#define FIRST_ROT_TITLE	(1.85f)		// タイトル時の開始時の視点と注視点の間の向き (x)
#define FIRST_DIS_TITLE	(-800.0f)	// タイトル時の開始時の視点と注視点の間の距離
#define FIRST_ROT_GAME	(2.3f)		// ゲーム時の開始時の視点と注視点の間の向き (x)
#define FIRST_DIS_GAME	(-680.0f)	// ゲーム時の開始時の視点と注視点の間の距離

#define MOVE_ROT_TITLE	(0.0032f)	// タイトル時のカメラの向き自動変更量

#define POS_R_PLUS		(25.0f)		// プレイヤーの位置と注視点の位置の距離
#define POS_V_Y			(400.0f)	// 追従時の視点の位置 (y)
...
//	プロトタイプ宣言
//**********************************************************************************************************************
void TitleCamera(void);				// タイトルのカメラの更新処理
void GameCamera(void);				// ゲームのカメラの更新処理
...
void InitCamera(void)
{
	...
	case MODE_TITLE:	// タイトル画面
		...
		g_camera.rot      = D3DXVECTOR3(FIRST_ROT_TITLE, 0.0f, 0.0f);	// 向き
		g_camera.fDis     = FIRST_DIS_TITLE;							// 視点と注視点の距離
...
#if 1
		// カメラの情報を初期化
		g_camera.posV     = D3DXVECTOR3(0.0f, POS_V_Y, 0.0f);			// 現在の視点
		g_camera.posR     = D3DXVECTOR3(0.0f, 0.0f, 0.0f);				// 現在の注視点
		g_camera.destPosV = D3DXVECTOR3(0.0f, 0.0f, 0.0f);				// 目標の視点
		g_camera.destPosR = D3DXVECTOR3(0.0f, 0.0f, 0.0f);				// 目標の注視点
		g_camera.vecU     = D3DXVECTOR3(0.0f, 1.0f, 0.0f);				// 上方向ベクトル
		g_camera.rot      = D3DXVECTOR3(FIRST_ROT_GAME, 0.0f, 0.0f);	// 向き
		g_camera.fDis     = FIRST_DIS_GAME;								// 視点と注視点の距離
#else
		// カメラの情報を初期化
		g_camera.posV     = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			// 現在の視点
		g_camera.posR     = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			// 現在の注視点
